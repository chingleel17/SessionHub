...
- [x] 2.1 啟動應用程式，釘選同一 repo 的兩個不同分支，確認兩者均出現在 Sidebar 釘選區且互不取消
- [x] 2.2 重新啟動應用程式，確認兩個分支的釘選狀態均從 settings.json 正確還原
- [x] 2.3 確認舊格式（僅路徑，無 `:branch` 後綴）的釘選項目在升級後仍可正常顯示（向後相容）
