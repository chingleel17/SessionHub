...
新增 `searchInContent: boolean` state 與 `contentMatchIds: Set<string> | null` state。當 `searchInContent` 為 true 且關鍵字非空時，以 300ms debounce 呼叫指令，結果存入 `contentMatchIds`。`filterAndSortSessions` 新增一個 `contentMatchIds` 參數：為 `null` 時（未啟用或未回傳）不參與篩選，非 `null` 時要求 `contentMatchIds.has(session.id)`。函式本身仍是同步純函式。
