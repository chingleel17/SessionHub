...
- claude 與 copilot：無原生旗標。停用 SHALL 將該 server 自設定檔移除並將原始設定值暫存至 `%APPDATA%\SessionHub\mcp-disabled.json`（結構 `{"<provider>": {"<name>": <設定值>}}`）；啟用 SHALL 將暫存值原樣寫回設定檔並自暫存移除。清單 SHALL 合併顯示暫存中的停用項目（`enabled: false`）。
...
後端 SHALL 提供四個 Tauri commands，皆回傳 `Result<T, String>` 並以 `spawn_blocking` 執行檔案 I/O：

- `list_mcp_configs() -> Vec<McpProviderConfig>`：回傳四個平台的 `{ providerId, configPath, configExists, servers: [{ name, enabled, configJson }], error? }`
- `upsert_mcp_server(provider, name, originalName?, configJson)`
- `delete_mcp_server(provider, name)`
- `set_mcp_server_enabled(provider, name, enabled)`

前端 IPC 呼叫 SHALL 集中於 `App.tsx`，`McpConfigView` 為純顯示元件；所有操作成功／失敗 SHALL 以 toast 回饋，介面文字 SHALL 全部經 i18n（zh-TW 與 en-US）。
