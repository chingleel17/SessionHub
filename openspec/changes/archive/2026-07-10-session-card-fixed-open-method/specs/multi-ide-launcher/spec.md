...
**Reason**: session 的開啟方式不應可自由選擇——各 provider 的 session 只能以對應 CLI resume；自由選擇工具的需求屬於專案路徑層級。
**Migration**: 專案層級的多工具開啟選單由新 capability `project-launcher` 提供（位於 ProjectView 頂部）；session 開啟改為 provider 綁定行為（見下方 MODIFIED）。
