...
系統 SHALL 在左側面板提供可切換的 Explorer 導覽模式，用於顯示所有可瀏覽的 Sisyphus 與 OpenSpec 文件節點，且三種模式皆須保留來源層級（Sisyphus / OpenSpec）作為最外層。
...
- **THEN** 來源層級（Sisyphus、OpenSpec）預設展開
- **AND** 各來源層級底下僅 `Plans` 與 `Active Changes` 群組預設展開
- **AND** `Notepads`、`Drafts`、`Archived Changes`、`Specs` 等其他群組與子群組預設折疊
- **AND** 使用者手動展開或折疊來源層級或群組後，該狀態在當前頁面 session 內維持

#### Scenario: Tree 模式顯示階層導覽

- **WHEN** 使用者切換到 `Tree` 模式
- **THEN** 左側以多層樹狀結構顯示來源節點、群組節點與葉節點
- **AND** 來源節點與群組節點仍可獨立展開或折疊
...
- **WHEN** 使用者切換到 `List` 模式
- **THEN** 左側先以可折疊的來源層級標頭區分 Sisyphus 與 OpenSpec
- **AND** 各來源層級底下以列表列（row）方式呈現其群組內容，而非帶陰影的卡片
- **AND** 每個 OpenSpec change 為一個獨立列區塊，列之間以間距區分
- **AND** OpenSpec change 列第一行顯示 change 名稱，並在該行最右側顯示所屬 spec 數量（如 `2 specs`）
...
- **WHEN** 使用者切換到 `Cols` 模式
- **THEN** 左側以兩欄呈現：第一欄為依來源層級分組的狀態群組清單，第二欄為所選群組內的項目
- **AND** 第一欄的來源層級標頭可獨立展開或折疊
- **AND** 第一欄一次僅展開一個狀態群組（預設為 OpenSpec 的 `Active Changes`；若專案僅有 Sisyphus 資料則為 `Plans`），選取其他群組時收合前一個
- **AND** 第二欄每個 OpenSpec change 列顯示 `done/total` 進度 badge 與進度條
- **AND** 點擊最終可讀取檔案後，右側面板顯示對應內容

#### Scenario: Cols 模式 Active Changes 顯示待辦動作徽章

- **WHEN** Cols 模式第二欄呈現 OpenSpec 來源層級下 `Active Changes` 群組內的 change 項目
...
- **WHEN** Cols 模式第二欄呈現 OpenSpec 來源層級下 `Archived Changes` 群組內的 change 項目
