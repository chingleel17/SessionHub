...
#### Scenario: herdr 模式改由建立 tab 時聚焦

- **WHEN** 啟動器為 herdr 且系統建立新的 tab
- **THEN** 系統在建立 tab 時即要求聚焦該 tab
- **AND** 不執行依視窗標題比對的 Win32 聚焦流程

#### Scenario: herdr 模式下的聚焦終端操作

- **WHEN** 啟動器為 herdr 且使用者觸發「聚焦終端」
- **THEN** 系統不以視窗標題比對定位目標
- **AND** 若無法定位對應 pane，回傳明確錯誤訊息由前端以 toast 呈現
