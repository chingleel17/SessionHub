...
- [ ] 2.2 將現有 shell 啟動邏輯抽為 `launch_via_shell`，保留 file_stem 白名單、`current_dir`、`CREATE_NEW_CONSOLE` 與 `configure_msys_stackdump_suppression`
...
- [ ] 3.1 新增 herdr 模組，實作 `herdr_tab_create(cwd, label, focus)`：以 `output()` 執行 `herdr tab create --cwd <PATH> --label <TEXT> --focus`，解析 stdout JSON 取 `result.root_pane.pane_id`
...
- [ ] 4.2 herdr 模式下不進入 EnumWindows 比對，回傳可辨識的錯誤訊息供前端 toast 呈現
- [ ] 4.3 確認 shell 模式的既有 Win32 聚焦邏輯完全不變

## 5. 終端路徑驗證分流

- [ ] 5.1 `validate_terminal_path` command 支援依 launcher 分流（新增參數或對應 command），並於 `commands/mod.rs`、`lib.rs` 同步登記
...
- [ ] 6.2 `src/App.tsx` 傳遞 launcher 至相關 `invoke()` 呼叫（`open_in_tool`、`resume_session_in_terminal`、`focus_terminal_window`、`validate_terminal_path`）
...
- [ ] 7.6 手動驗證 herdr 模式：開終端建立 tab、啟動 AI CLI 於 pane 執行、tab 標籤可辨識、聚焦錯誤訊息正確呈現
