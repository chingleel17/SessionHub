...
提供常駐桌面、永遠置頂且不搶焦點的 quota 用量顯示體驗，讓使用者無需切換到主視窗即可掌握各 provider（Claude、Codex、Copilot、OpenCode 等）的剩餘額度，涵蓋系統匣圖示動態顯示、桌面 Overlay Widget、Tray 點擊彈出 Mini Panel 三層顯示與對應的顯示設定。
