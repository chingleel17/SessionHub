...
定義 Commands 在專案與全域範圍的來源掃描、目標目錄對映、狀態矩陣與同步行為。
...
- **WHEN** 某 command 列是因「另一個 target 端存在同名檔案」而被反查列出（來源尚未建立），而目前這一欄的 target 也不存在該檔案
- **THEN** 系統 SHALL 將該欄狀態標示為「缺少目標」（target-missing），與一般「來源存在但此 target 尚未同步」的情況一致對待
- **AND** 系統 SHALL NOT 將此情況標示為「錯誤」（error）狀態，error 狀態僅保留給檔案讀寫或連結解析等真正的例外錯誤，不可用於「來源與目標單純皆不存在」的正常情境
- **AND** 此修正用於避免將尚未同步的正常狀態誤顯示為錯誤
