...
Session 清單 SHALL 支援多維度等過濾條件的組合，包含 provider、是否顯示空對話、文字搜尋與更新時間區間。排序選項 SHALL 限定為更新時間、建立時間與標題三種。

#### Scenario: 過濾空 session

- **WHEN** 使用者未啟用「空對話」chip（預設狀態）
- **THEN** `hasEvents` 為 false 的 session SHALL 被隱藏
...
- **WHEN** 使用者將更新時間篩選切換為「本周」、「本月」或「自訂區間」
...
- **WHEN** 使用者開啟排序選單
- **THEN** 選單 SHALL 只提供「依更新時間」、「依建立時間」、「依標題」三個選項
- **AND** SHALL NOT 提供依 summary 數量排序的選項
...
- **WHEN** 使用者修改搜尋、排序、tag、provider、空對話顯示、更新時間篩選或對話內容搜尋開關
- **THEN** 系統自動切回第 1 頁
- **AND** 以新的篩選結果重新計算總頁數
