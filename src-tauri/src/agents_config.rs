...
use crate::resource_discovery::{
    ...
    skill_roots as discovery_skill_roots, DiscoveryDiagnostic, DiscoveryScope, DiscoverySource,
    ResourceDiscovery, ResourceKind, ResourceLocation, ResourceState,
};
use crate::settings::{
...
use crate::token_estimator::{estimate_mixed_text, FileContentMetrics};
use crate::types::{
...
pub(crate) struct AgentsMdEntry {
    ...
    pub(crate) source_metrics: Option<FileContentMetrics>,
    pub(crate) target_metrics: Option<FileContentMetrics>,
...
pub(crate) struct SkillEntry {
    ...
    pub(crate) metrics: Option<FileContentMetrics>,
}
...
pub(crate) fn scan_global_agents_md_internal() -> Result<AgentsMdScanResult, String> {
    ...
    for (_, root) in roots {
        ...
        let source_metrics = content_metrics_for_file(&source);
        let target_metrics = content_metrics_for_file(&target);
...
pub(crate) fn scan_agents_skills_internal(scope: &AgentsScope) -> Result<SkillsScanResult, String> {
    ...
    let skills = discovered
        ...
        .map(|(name, discovered_dir)| {
            ...
            let statuses = targets
                .iter()
                .map(|(target_id, target_root)| {
                    compare_directory_target(
                        target_id,
                        target_root,
                        &skill_dir,
            ...
            let metrics = content_metrics_for_file(&fingerprint_file(&skill_md_path));
            Ok(SkillEntry {
                name,
                ...
                targets: statuses,
                description: read_frontmatter_description(&skill_md_path),
                provider_id: None,
                locations: Vec::new(),
                effective_path: None,
                cli_state: None,
    ...
    Ok(SkillsScanResult {
        ...
        enabled_providers: vec![CLAUDE_PROVIDER.to_string()],
        discoveries: Vec::new(),
        diagnostics: Vec::new(),
    })
}

pub(crate) fn scan_agents_skills_with_providers(
    scope: &AgentsScope,
    enabled_providers: &[String],
) -> Result<SkillsScanResult, String> {
    ...
    let roots = discovery_skill_roots(
        match scope {
            AgentsScope::Project { .. } => DiscoveryScope::Project,
            AgentsScope::Global => DiscoveryScope::Global,
        },
        project_cwd,
        &settings,
        &enabled_providers,
    ...
    let canonical_root = match scope {
        AgentsScope::Project { project_cwd } => {
            PathBuf::from(project_cwd).join(".agents").join("skills")
        }
        AgentsScope::Global => {
            resolve_agents_source_root(Some(&settings.agents_source_root))?.join("skills")
        }
    };
    ...
    let mut diagnostics = unknown_providers
        .iter()
        .map(|provider| DiscoveryDiagnostic {
    ...
    for provider in &enabled_providers {
        ...
        let cli_resources = if provider == OPENCODE_PROVIDER {
            match discover_cli(
                ...
                project_cwd.and_then(|path| path.to_str()),
            ) {
        ...
        for (name, locations) in discovered {
            ...
            let source_dir = if canonical_root.join(&name).join(SKILL_FILE_NAME).is_file() {
                canonical_root.join(&name)
            } else {
                primary.clone()
            };
            let source_fp = fingerprint_directory(&source_dir, None, &[])?;
            let location_items = locations
                .iter()
                ...
                .collect::<Vec<_>>();
            let cli = cli_resources
                .iter()
                .find(|item| item.name.eq_ignore_ascii_case(&name));
            let statuses = target_roots
                .iter()
                .map(|(target_id, target_root)| {
                    compare_directory_target(
                        target_id,
                ...
                .collect::<Result<Vec<_>, _>>()?;
            let skill_md_path = source_dir.join(SKILL_FILE_NAME);
            skills.push(SkillEntry {
                ...
                metrics: content_metrics_for_file(&fingerprint_file(&skill_md_path)),
            });
        }
        let discovered_names = skills
            .iter()
            .filter(|skill| skill.provider_id.as_deref() == Some(provider))
        ...
        for cli in cli_resources.iter().filter(|item| {
            ...
            skills.push(SkillEntry {
                ...
                metrics: cli
                    .effective_path
                    .as_deref()
                    .and_then(|path| content_metrics_for_file(&fingerprint_file(Path::new(path)))),
            });
        }
    }
    let discoveries = skills
        .iter()
        .filter_map(|entry| {
            entry
                .provider_id
                .as_ref()
                .map(|provider| ResourceDiscovery {
                    provider_id: provider.clone(),
                    kind: ResourceKind::Skill,
                    scope: match scope {
                        AgentsScope::Project { .. } => DiscoveryScope::Project,
    ...
    return Ok(SkillsScanResult {
        source_root: normalize_display_path(
            &roots
                .first()
    ...
    for (provider, root) in roots {
            ...
            .filter_map(Result::ok)
        {
            ...
            let metadata = fs::metadata(path).map_err(|error| {
                format!(
                    "failed to read resource metadata {}: {error}",
                    path.display()
                )
            })?;
            ...
            hasher.update(
                system_time_to_millis(metadata.modified().ok().unwrap_or(UNIX_EPOCH.into()))
                    .unwrap_or(0)
                    .to_le_bytes(),
            );
        }
...
pub(crate) fn scan_agents_commands_internal(
    ...
    if source_root.is_dir() {
            ...
            .filter_map(Result::ok)
        {
            ...
            if let Some(name) = strip_command_file_suffix(relative, "") {
    ...
    for (target_id, target_root) in &targets {
            ...
            .filter_map(Result::ok)
        {
            ...
            let Some(relative) = path.strip_prefix(target_root).ok() else {
                continue;
            };
            if let Some(name) = strip_command_file_suffix(relative, target_id) {
                discovered.entry(name).or_insert_with(|| path.to_path_buf());
            }
        }
    }
        ...
        .map(|(name, preview_path)| {
            ...
            let statuses = targets
                ...
                .map(|(target_id, target_root)| {
                    ...
                    let (status, target_newer) =
                        classify_file_status(&source_fp, &fingerprint_file(&target_path));
                    TargetStatus {
                        target_id: target_id.clone(),
                ...
                .collect();
            let metrics = content_metrics_for_file(&fingerprint_file(&preview_source_path));
            Ok(CommandEntry {
                name,
                source_path: normalize_display_path(&preview_source_path),
                sync_source_path: normalize_display_path(&sync_source_path),
                targets: statuses,
                description: read_frontmatter_description(&preview_source_path),
                provider_id: None,
                locations: Vec::new(),
                effective_path: None,
                cli_state: None,
...
pub(crate) fn scan_agents_commands_with_providers(
    ...
    let mut diagnostics = unknown_providers
        ...
        .collect::<Vec<_>>();
    for provider in &enabled_providers {
        let suffix = if provider == COPILOT_PROVIDER {
            ".prompt.md"
        ...
        for (_, root) in provider_roots {
                ...
                .filter_map(Result::ok)
            {
                ...
                found
                    .entry(name[..name.len() - suffix.len()].replace('\\', "/"))
                    .or_default()
        ...
        for (name, locations) in found {
            ...
            let source_fp = fingerprint_file(&sync_source_path);
            let preview_path = if source_fp.exists {
                sync_source_path.clone()
            } else {
                primary.clone()
            };
            ...
            commands.push(CommandEntry {
                ...
                cli_state: cli.map(|_| ResourceState::Available),
                cli_source: cli.map(|_| DiscoverySource::Cli),
                metrics: content_metrics_for_file(&fingerprint_file(&preview_path)),
            });
        }
        let command_names = commands
            .iter()
            .filter(|command| command.provider_id.as_deref() == Some(provider))
            .map(|command| command.name.to_lowercase())
            .collect::<std::collections::HashSet<_>>();
        for cli in cli_resources.iter().filter(|item| {
            include_cli_only_resources(scope) && !command_names.contains(&item.name.to_lowercase())
        }) {
            commands.push(CommandEntry {
                name: cli.name.clone(),
                source_path: cli.effective_path.clone().unwrap_or_default(),
                sync_source_path: normalize_display_path(
                    &canonical_root.join(command_relative_path(&cli.name, "")),
                ),
                targets: target_roots
                    .iter()
                    .map(|(target_id, target_root)| TargetStatus {
                        target_id: target_id.clone(),
                        target_root: normalize_display_path(target_root),
...
fn content_metrics_for_file(fingerprint: &FileFingerprint) -> Option<FileContentMetrics> {
    if !fingerprint.exists {
        return None;
    }

    fs::read_to_string(&fingerprint.path)
        .ok()
        .map(|content| estimate_mixed_text(&content))
}
