#!/usr/bin/env sh
# on-post-tool-use.sh - Claude PostToolUse hook
...
title="$(get_hook_string_value "tool_name" "toolName" "model")"
write_bridge_event_record "$BRIDGE_PATH" "$PROVIDER" "tool.post" "$title"
